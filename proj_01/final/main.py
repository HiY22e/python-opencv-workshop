...
def predict(imageArr):
    ...
    # cv2.imshow("img_edge3", img_edge4)
    # cv2.imshow("img_edge4", img_edge4)
    # img_edge4 img_edge4  show
...
def license_segment(img, car_plates, out_path):
    """
    ...
    if len(car_plates) == 1:
        for car_plate in car_plates:
            ...
            # cv2.imshow("card_img" + str(i) + ".jpg", card_img) show
            i += 1
            # cv2.waitKey(0)
...
def split_licensePlate_character(plate_binary_img):
    ...
    for i in range(len(split_list)):
        ...
        # cv2.imshow('character' + str(i), single_character_Arr) show
...
def classify(file_path):
    ...
    if len(car_plate) == 0:
        ...
        car_img_path = license_segment(img, car_plate, "img/LPR")
        ...
        # 配合pytesseract
        res_text = pytesseract_ocr(car_img_path)

        # cv2.waitKey(0)
        cv2.destroyAllWindows()

    label.configure(foreground='#011638', text=res_text)
    uploaded = Image.open("img/LPR/card_img0.jpg")  # 显示识别结果
    im = ImageTk.PhotoImage(uploaded)
    plate_image.configure(image=im)
    plate_image.image = im
    plate_image.pack()
    plate_image.place(x=690, y=320)


def show_classify_button(file_path):
    classify_b = Button(top, text="识别照片", command=lambda: classify(file_path), padx=10, pady=5)
    classify_b.configure(background='#364156', foreground='white', font=('arial', 15, 'bold'))
    classify_b.place(x=750, y=550)
...
if __name__ == "__main__":
    ...
    upload = Button(top, text="选择识别的照片", command=upload_image, padx=10, pady=5)
    upload.configure(background='#364156', foreground='white', font=('arial', 15, 'bold'))
    upload.pack()
    upload.place(x=100, y=550)
